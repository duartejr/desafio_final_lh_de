...
parser = argparse.ArgumentParser(
    description="Ingest stage data into the staging catalog."
)
logging.basicConfig(
...
def read_raw_df(
    catalog: str, schema: str, table_name: str, environment: str
) -> DataFrame:
    """
...
def load_stg_df(stg_df: DataFrame, catalog: str, schema: str, table_name: str) -> None:
    ...
    """
    spark.sql(f"USE CATALOG {catalog}")
    stg_df.write.format("delta").mode("append").saveAsTable(
        f"{catalog}.{schema}.{table_name}_stg"
    )
...
def insert_origin_columns(
    df: DataFrame, schema: str, database: str = "adventure_works"
) -> DataFrame:
    """
    ...
    df = df.withColumn("source_name", lit(database)).withColumn(
        "source_departament", lit(schema)
    )
    return df
    ...
    if raw_df.count():
        ...
        if table_exists:
            ...
            delta_table.alias("target").merge(
                stg_df.alias("source"), merge_condition
